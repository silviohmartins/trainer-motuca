...
3. Escolha o repositório GitHub e a licença antes de publicar o código-fonte. Nenhuma licença de código aberto foi escolhida automaticamente.
4. Copie a pasta `MotucaTrainer` do pacote para `%USERPROFILE%/Zomboid/Workshop/`. Dentro dela estão `Contents/mods/MotucaTrainer`, `preview.png` e `workshop.txt`.
5. Use a opção de envio à Workshop no menu principal do Project Zomboid e selecione essa pasta. Confira título, descrição e imagem no jogo antes de enviar. O pacote começa com visibilidade **privada** para revisão; escolha a visibilidade desejada no fluxo de publicação. A aceitação dos termos da Steam deve ser feita pelo titular da conta.
6. Opcionalmente, crie uma release GitHub marcada como **pré-lançamento**, versão `v0.2.0-beta.1`, anexando o ZIP e `SHA256SUMS.txt`.
7. Depois de obter o link da Workshop, acrescente-o à release GitHub e publique para o grupo beta.
